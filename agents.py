...
# creating agents: every agent has a role, goal, backstory,tools,and llm, verbose, memory, and allow_delegation

# researcher agent
researcher = Agent(role="Technology Intelligence Specialist & Innovation Scout",
    goal="""
    ...
    before they become mainstream.""", llm=llm, tools=[google_search_tool],allow_delegation=True, verbose=True)


# writer agent
writer = Agent(role="Technology Storyteller & Innovation Chronicler",
    goal="""
    ...
    """, llm=llm, tools=[google_search_tool],allow_delegation=True, verbose=True)


# proofreader agent
proofreader = Agent( role="Principal Proofreader",
